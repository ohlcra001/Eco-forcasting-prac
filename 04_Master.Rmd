...
>*Answer 1:*
My theory is that either the plants distributing and building the seed banks begin to die off after an extended period therfore limiting the amount of seeds, or, once the seeds begin to germinate there's an intrinsic carrying capacity limiting the amount of greenery and plants even if the seed banks kept building throughout the years.
<br>
...
>*Answer 2:* Alpha which represents NDVI right after fire has decreased in the more complicated model, showing the simple model perhaps overestimated greenary values for time 0. However the gamma value has increased in the second graph indicating a stronger emphasis on the maximum change of NDVI due to time rather than its initial value compared to the simpler model. Lamda or the rate of increase of NDVI has also decreased slightly in the more complicated model, meaning even though the maximum change is larger than the simple model, its starting value is much lower and so the rate ends up lower to the simple model.
<br>
...
>*Answer 3:*
It would appear model 2 has slightly less residual sum of squares normally indicating a better model, it certainly has a stronger fit. However when analysing models we must consider other factors which might come into play such as over-fitting.
<br>

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4:*
2 degrees of freedom, as we are using the two models as comparisons 
<br>
...
>*Answer 5:*
In this case the answer is much more clear, model 2 has a much lower AIC clearly indicating it as the preferred model.
<br>
...
>*Answer 6:*
AIC can only be used to compare models which are nested within one or another or some variation of another.It uses log-likelihood and maximum likelihood, therefore for a model like NLS, the AIC can be calculated but shouldnt be compared to a model which uses Maximum likelihood such as our MLE funcitons
<br>